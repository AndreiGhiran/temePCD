...
export default class TodoListController extends WebcController {
    ...
    _deleteItem = (event) => {
        const itemToDelete = this.model.items.find(item => item.delete?.name === event.target.name);
    ...
    removeListItem(todo) {
        ...
        this.TodoManagerService.removeToDo(todo, (err, data) => {
            if (err) {
                ...
                const updatedItems = this.model.items.filter(item => todo.input.name !== item.input.name);
